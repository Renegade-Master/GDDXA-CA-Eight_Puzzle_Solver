...
class Board implements Comparable<Board> {
	...
	public Iterable<Board> neighbours() {
		...
		// Is 0 the Last Tile?
			...
			//  Is the Blank on the Leftmost Column?
				...
				|| (((this.m_order % this.m_zeroTile) == this.m_order)
					&& this.m_zeroTile < this.m_order)) {
			...
			//  Is the Blank on the Rightmost Column?
					...
					|| (((this.m_order % this.m_zeroTile) == 1)
					&& this.m_zeroTile > (this.m_tiles.length - this.m_order))) {
				possibleBoards--;
				leftSafe = true;
			}
	...
	int zeroRow() {
		if(this.m_zeRow == Integer.MIN_VALUE) {
			...
			if (this.m_zeroTile >= (this.m_tiles.length - this.m_order)) {
				this.m_zeRow = 1;
				return(this.m_zeRow);
			}

			// Otherwise
			this.m_zeRow = Math.abs((int)Math.ceil(
					((double)this.m_zeroTile / (double)this.m_order))
					- this.m_order);
			this.m_zeRow += 1;
